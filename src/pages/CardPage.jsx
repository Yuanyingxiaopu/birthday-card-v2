...
export default function CardPage() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: '🎂 专属生日贺卡',
          text: `${card?.sender || '有人'}给你发了一张生日贺卡，快来看看吧！`,
          url: shareUrl,
        })
      } catch {}
    } else {
  ...
  const handleRegenerate = () => {
    sessionStorage.removeItem('card_data')
    ...
    <div className="flex-1 flex flex-col items-center py-6 px-4">
      ...
      >
        ...
        <div className={`text-center py-8 px-4 transition-all duration-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <h1
            className="text-2xl font-bold text-pink-600 tracking-wider"
        ...
        <div className={`text-center px-6 mb-4 transition-all duration-700 delay-200 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <h2 className="text-2xl font-bold text-pink-600" style={{ fontFamily: 'var(--font-display)' }}>
            {card.name}
          </h2>
          ...
          <div className="text-center">
            ...
            <div className="space-y-2">
              {blessingLines.map((line, i) => (
                <p key={i} className="text-base text-pink-700 font-medium" style={{ fontFamily: 'var(--font-display)' }}>
                  {line}
                </p>
              ))}
            </div>
        ...
        {card.sender && (
          <div className={`text-center px-6 mb-6 transition-all duration-700 delay-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <p className="text-sm text-pink-400">
              —— 来自 <span className="font-semibold text-pink-500">{card.sender}</span> 的祝福
            </p>
          </div>
        )}
      ...
      <div className={`w-full max-w-sm mt-6 space-y-3 transition-all duration-700 delay-1000 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        <button
          onClick={handleSave}
          className="w-full py-3.5 bg-gradient-to-r from-pink-500 to-pink-400 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2"
        >
          📥 保存图片
        </button>
        <div className="flex gap-3">
          <button
            onClick={handleShare}
            className="flex-1 py-3 bg-white/80 border-2 border-pink-200 text-pink-600 font-semibold rounded-xl hover:bg-pink-50 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
          >
            💌 分享好友
          </button>
          <button
